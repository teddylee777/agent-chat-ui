...
import {
  getAgentMiddlewareConfig,
  updateAgentMiddlewareConfig,
  validateAgentMiddlewareConfig,
} from "@/lib/api/agent-builder";
import type { AgentMiddlewareConfigResponse } from "@/lib/types/agent-builder";
import { toast } from "sonner";

interface MiddlewareSettingsDialogProps {
  agentId: string;
  middlewareName: string;
}

export function MiddlewareSettingsDialog({
  agentId,
  middlewareName,
}: MiddlewareSettingsDialogProps) {
  const [isOpen, setIsOpen] = useState(false);
  const [isLoading, setIsLoading] = useState(false);
  const [isSaving, setIsSaving] = useState(false);
  const [configData, setConfigData] = useState<AgentMiddlewareConfigResponse | null>(
    null
  );
  ...
  useEffect(() => {
    async function loadConfig() {
      ...
      try {
        const data = await getAgentMiddlewareConfig(agentId, middlewareName);
        setConfigData(data);
        setEditedOverride(data.config_override || {});
        setJsonText(JSON.stringify(data.config_override || {}, null, 2));
      } catch (err) {
        toast.error("Failed to load middleware config");
        setIsOpen(false);
      } finally {
        setIsLoading(false);
      }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Validate first
      const validationResult = await validateAgentMiddlewareConfig(
        agentId,
        middlewareName,
        finalOverride
      );
      ...
      // Save
      await updateAgentMiddlewareConfig(agentId, middlewareName, finalOverride);
      toast.success("Middleware settings saved successfully");
      setIsOpen(false);
    } catch (err) {
      toast.error("Failed to save middleware settings");
    } finally {
      setIsSaving(false);
    }
  };
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <DialogTitle>Middleware Settings: {middlewareName}</DialogTitle>
        </DialogHeader>
